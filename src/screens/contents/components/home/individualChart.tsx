...
function IndividualChart({ incidentType, year }: Props) {
  const [data, setdata] = useState<reportdata[]>([]);
  useEffect(() => {

    const fetchData = async () => {
      try {
        const Accidents: reportdata[] = await fetchreport('incident', incidentType) || [];
    ...
    <>
      <BarChart
        ...
        series={[
          ...
          { data: processeddata, label: incidentType, color: '#FE0000'},
        ]}
        width={1500}
        height={300}
      />
